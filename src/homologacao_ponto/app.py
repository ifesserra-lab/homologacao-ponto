...
from homologacao_ponto.infrastructure.sigrh_browser import BrowserSetupError, SigrhBrowser
...
CURRENT_ESPELHO_PONTO_URL = (
    "https://sigrh.ifes.edu.br/sigrh/frequencia/ponto_eletronico/consulta/consulta_ponto_eletronico.jsf"
)
...
class AppResult:
    exit_code: int
    message: str
    result: CrawlResult | NavigationResult | SelecaoServidorResult | ExportacaoEspelhoResult | None = None
...
class HomologacaoPontoApp:
    ...
    def run_espelho_ponto(
        self, servidor: str | None = None, mes: int | None = None, ano: int | None = None, siape: str | None = None
    ) -> AppResult:
        try:
            credential = self.credential_provider.load()
            login = self.auth_service.login(credential)
    ...
    def _try_direct_espelho_navigation(self, result: NavigationResult) -> bool:
        ...
        if self.browser.is_anti_automation(snapshot.html) or self.browser.is_session_expired(snapshot.html):
            return False
    ...
    def _export_app_result(result: ExportacaoEspelhoResult) -> AppResult:
        result_suffix = f" Resultado: {result.output_path}" if result.output_path else ""
...
def create_app(output_dir: Path | str, env_file: Path | str, headed: bool = False) -> HomologacaoPontoApp:
